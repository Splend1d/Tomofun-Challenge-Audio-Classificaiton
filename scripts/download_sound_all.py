...
import subprocess
import os
df = pd.read_csv("/mnt/HDD/competitions/tomofun/data/audioset/audioset_train_strong_with_label.tsv",sep = "\t")
print(df)
...
for row in df.itertuples():
	...
	if os.path.exists(filename):
		...
		continue
	cmd = ["bash", "./scripts/download_sound.sh",page,str(start),str(duration),filename]
	p = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
	try:
		outs, errs = p.communicate(timeout=25)
